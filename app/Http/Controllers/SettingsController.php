    ...
    public function companyslate(Request $request)
    {
        if(\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin')
        {
            if($request->file)
            {
                ...
                $logoName  = 'company_slate_'.date("Y").'.pdf';
            ...
            if($request->file_ar)
            {
                $request->validate(
                [
                    'file_ar' => 'file|mimes:pdf',
                ]);

                $logoName  = 'company_slate_ar_'.date("Y").'.pdf';
                $title_ar  = pathinfo($request->file('file_ar')->getClientOriginalName() , PATHINFO_FILENAME);
                $path2     = $request->file('file_ar')->storeAs('uploads', $logoName);
            }

            Companyslate::updateOrCreate([
                'created_by'  =>  \Auth::user()->creatorId(),
            ],[
                'title'       =>  $title,
                'title_ar'    =>  $title_ar,
            ...
            return redirect()->back()->with('success', 'Company Slate successfully updated.');
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }

    public function companyslateAccept(Request $request)
    {
