        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5>{{__('Sub Employee Request Type')}}</h5>
                </div>
                @if (session('success'))
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover dataTables">
                            ...
                            <tbody class="font-style">
                                @foreach ($sub_request_types as $request_type)
                                    <tr>
                                        <td>{{ $request_type->name ??'' }}</td>
                                        <td>{{ $request_type->name_ar ??'' }}</td>
                                        <td>{{ $request_type->requestType->name ??'' }}</td>
                                        <td class="text-right">
                                            <!-- Edit Button -->
                                            @can('Edit Employee')
                                                <a href="{{ URL::to('sub-request-type/'.$request_type->id.'/edit') }}"
                                                   class="btn btn-success btn-icon-only"
                                                   data-url="{{ URL::to('sub-request-type/'.$request_type->id.'/edit') }}"
                                                   data-size="lg"
                                                   data-ajax-popup="true"
                                                   data-title="{{ __('Edit Request Type') }}"
                                                   data-toggle="tooltip"
                                            ...
                                            @can('Delete Employee')
                                                <a href="#"
                                                   ...
                                                   onclick="if(confirm('{{ __('Are you sure? This action cannot be undone.') }}')) {
                                                                document.getElementById('delete-form-{{ $request_type->id }}').submit();
                                                            }">
                                                    <i class="fas fa-trash-alt"></i>
                                                </a>

                                                <!-- Form for Delete -->
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['sub-request-type.destroy', $request_type->id], 'id' => 'delete-form-' . $request_type->id, 'style' => 'display:none;']) !!}
                                                {!! Form::close() !!}
                                            @endcan
                                        </td>
                                    </tr>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createNationalityModalLabel">{{__('sub request type')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    {{ Form::open(['url' => 'sub-request-type', 'method' => 'post']) }}
                    <div class="row">
                        <!-- Name Field -->
                        <div class="col-6">
