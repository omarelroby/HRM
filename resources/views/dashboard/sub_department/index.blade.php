    ...
    <div class="row">
        ...
        <div class="d-flex justify-content-end mb-3">
                 <a href="#" data-bs-toggle="modal" data-bs-target="#addTrainingModal" class="btn btn-primary btn-lg">
                    <i class="fas fa-plus"></i> {{ __('Create New Sub Department') }}
                </a>
        ...
        <div class="col-lg-12">
            <div class="card shadow-sm">
                <div class="card-header text-white">
                    <h5 class="card-title mb-0">{{ __('Sub Departments') }}</h5>
                </div>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables">
                            ...
                            <tbody class="font-style">
                                @foreach ($sub_deps as $sub)
                                    <tr>
                                        <td>{{ !empty($sub->department_id)?$sub->department->name:'' }}</td>
                                        <td>{{ $sub->name ??''}}</td>
                                        <td>{{ $sub->name_ar ??''}}</td>
                                        ...
                                        <td class="text-right action-btns">
                                                ...
                                                <a href="{{ route('sub-department.edit',$sub->id) }}"
                                                ...
                                                <form method="POST" action="{{ route('sub-department.destroy', $sub->id) }}" class="d-inline" onsubmit="return confirm('{{ __('Are You Sure?') }}\n{{ __('This action cannot be undone. Do you want to continue?') }}');">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {{Form::open(array('url'=>'sub-department','method'=>'post'))}}
                <div class="row ">
                    <div class="col-12">
                        <div class="form-group">
                            {{Form::label('department_id',__('Branch'),['class'=>'form-control-label'])}}
                            {{Form::select('department_id',$departments,null,array('class'=>'form-control select2','placeholder'=>__('Select Sub Department')))}}
                        </div>
                    </div>
{{--
                    <div class="col-md-12">
