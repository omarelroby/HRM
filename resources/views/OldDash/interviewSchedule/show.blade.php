    ...
    <div class="card-body">
        ...
        @foreach($stages as $stage)
            <div class="custom-control custom-radio">
                <input type="radio" id="stage_{{$stage->id}}" name="stage" data-scheduleid="{{$interviewSchedule->candidate}}" value="{{$stage->id}}" class="custom-control-input stages" {{!empty($interviewSchedule->applications)?!empty($interviewSchedule->applications->stage==$stage->id)?'checked':'':''}}>
                <label class="custom-control-label" for="stage_{{$stage->id}}">{{$stage->title}}</label>
            </div>
        @endforeach
    </div>
    <div class="card-body">
        <a href="#" data-url="{{route('job.on.board.create', $interviewSchedule->candidate)}}"  data-ajax-popup="true"  class="btn btn-primary btn-icon-only width-auto" >  {{__('Add to Job OnBoard')}}</a>
    </div>
</div>
