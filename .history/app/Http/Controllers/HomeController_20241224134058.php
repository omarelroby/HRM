    ...
    public function index()
    {
        if (Auth::check()) {
            ...
            $data['late_arrivals'] = AttendanceEmployee::where('status', 'Present')
                ...
                $data['openings'] = Job::select('id', 'title', 'category', 'position')->get();

                $data['applicants'] = JobApplication::select('id', 'name', 'email', 'phone', 'profile', 'resume', 'rating', 'gender', 'country', 'state', 'city', 'job')
                ->get();
