...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('Create Employee')) {
            ...
            // File Uploads
            $fileFields = ['iqama_attachment', 'passport_attachment', 'insurance_document'];
            foreach ($fileFields as $field) {
                if ($request->hasFile($field)) {
                    $file = $request->file($field);
                    $fileName = $field . '_' . time() . '.' . $file->getClientOriginalExtension();
                    $path = $file->storeAs('uploads/document/', $fileName);
                    $data[$field] = $fileName;
                }
            }
            ...
            return response()->json([
                'status'  => 'success',
                'message' => __('Employee successfully created.') . (isset($smtp_error) ? ' ' . $smtp_error : ''),
            ]);
        } else {
            return response()->json([
                'status'  => 'error',
    ...
    public function edit($id)
    {
        ...
        'jobclasses'        => ['1', '2'], // Static array for job classes
        'banks'             => Bank::where('created_by', $creatorId)->get(),
        'jobtitles'         => Jobtitle::where('created_by', $creatorId)->get(),
        'job_types'         => Jobtype::where('created_by', $creatorId)->get(),
        'work_units'        => Workunit::where('created_by', $creatorId)->get(),
        'roles'             => Role::where('created_by', $creatorId)->get(),
        'laborCompanies'    => Laborhirecompany::where('created_by', $creatorId)->get(),
        ...
        'employee'          => Employee::findOrFail($id), // Current employee details
    ...
    public function show($id)
    {
        if(\Auth::user()->can('Show Employee'))
        {
            ...
            return view('daemployee.show', compact('employee','lang','setting','holidays','employees','assets','documents','employeesAttendance','dates', 'data',
