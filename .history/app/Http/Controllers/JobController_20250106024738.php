...
class JobController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->can('Manage Job Category'))
        {
            ...
            $data['categories'] = JobCategory::where('created_by', \Auth::user()->creatorId())->get()->pluck('title', 'id');
            $data['categories']->prepend('--', '');

            $data['branches'] = Branch::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            $data['branches']->prepend('All', 0);

            $data['status'] = Job::$status;

            $data['customQuestion'] = CustomQuestion::where('created_by', \Auth::user()->creatorId())->get();

            return view('dashboard.job.index', compact('jobs', 'data'));
        }
        else
        {
    ...
    public function edit(Job $job)
    {
        ...
        return view('dashboard.job.edit', compact('categories', 'status', 'branches', 'job', 'customQuestion'));
    }
