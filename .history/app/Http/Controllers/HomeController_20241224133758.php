    ...
    public function index()
    {
        if (Auth::check()) {
            ...
            $data['late_arrivals'] = AttendanceEmployee::where('status', 'Present')
                ...
                $openings = Job::select('id', 'title', 'position')
                ->where('status', 'open') // Assuming 'open' indicates active jobs
                ->get();

    // Fetch applicants grouped by job
    $applicants = JobApplicant::select('job', 'name', 'phone', 'profile', 'rating', 'skill', 'country', 'state', 'city', 'gender', 'dob')
        ->with('jobRelation') // Assuming you have defined the relation in your JobApplicant model
        ->get();
