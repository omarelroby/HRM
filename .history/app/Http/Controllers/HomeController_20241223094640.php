...
class HomeController extends Controller
{
    ...
    ->get()
    ->groupBy('name') // Group by department name to aggregate results
    ->map(function ($group) {
        return [
            'name' => $group->first()->name, // Take the first instance's name
            'total_employees' => $group->sum('employee_id'), // Sum up the employee counts
        ];
    })
    ->values(); // Reset indices

$data['departmentNames'] = $departments->pluck('name'); // Unique department names
$data['employeeCounts'] = $departments->pluck('total_employees'); // Summed employee counts
