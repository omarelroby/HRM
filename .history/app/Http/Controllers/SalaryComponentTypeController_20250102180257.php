...
class SalaryComponentTypeController extends Controller
{
    public function index()
    {
        if(\Auth::user()->can('Manage Employee'))
        {
            ...
            return view('dashboardsalary_components_types.index', compact('salary_components_types'));
        }
        else
        {
    ...
    public function edit($salary_components_type)
    {
        if(\Auth::user()->can('Edit Employee'))
        {
            ...
            {
                return view('dashboard.salary_components_types.edit', compact('salary_components_type'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
