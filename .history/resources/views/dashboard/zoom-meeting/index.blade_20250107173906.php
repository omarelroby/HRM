                                ...
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <div class="actions ml-3 rtl-actions">
                                            ...
                                            {{-- Delete Button --}}
                                            <a href="#" class="action-item text-danger mr-2 delete-icon" data-toggle="tooltip"
                                                data-original-title="{{ __('Delete') }}"
                                                data-confirm="{{ __('Are You Sure?') }}|{{ __('This action cannot be undone. Do you want to continue?') }}"
                                                onclick="confirmDelete('delete-form-{{ $ZoomMeeting->id }}')">
                                                <i class="fa fa-trash"></i>
                                            </a>

                                            {{-- Delete Form --}}
                                            {!! Form::open(['method' => 'DELETE', 'route' => ['zoom-meeting.destroy', $ZoomMeeting->id], 'id' => 'delete-form-' . $ZoomMeeting->id, 'class' => 'd-none']) !!}
                                            {!! Form::close() !!}

                                            <span class="clearfix"></span>

                                            <form method="POST" action="{{ route('zoom-meeting.destroy', $ticket->id) }}" class="d-inline" onsubmit="return confirm('{{ __('Are You Sure?') }}\n{{ __('This action cannot be undone. Do you want to continue?') }}');">
                                                @csrf
                                                @method('DELETE')
