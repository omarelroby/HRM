...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const chartElement = document.querySelector("#emp-department");

    let chart = null;

    function updateChart(period) {
        // Generate the URL using Laravel's route helper
        const url = `/get-department-data/${period}`;

        fetch(url)
            .then((response) => response.json())
            .then((data) => {
                const departmentNames = data.departmentNames;
                const employeeCounts = data.totalEmployees;
    ...
    dropdownItems.forEach((item) => {
        item.addEventListener("click", function () {
            ...
            updateChart(period); // Fetch and update chart
        });
    });

    // Load default data (e.g., "This Week")
