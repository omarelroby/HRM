...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(!employeeCounts.length || !departmentNames.length != employeeCounts.length) {
        }
        console.log("Department Names:", departmentNames);
        console.log("Employee Counts:", employeeCounts);

        if (departmentNames && departmentNames.length > 0 && employeeCounts && employeeCounts.length > 0) {
            var options = {
                series: [{
                    data: employeeCounts
                }],
