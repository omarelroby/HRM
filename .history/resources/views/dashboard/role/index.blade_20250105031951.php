    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.role && response.permissions) {
                // Update modal title and populate fields
                $('#EditRoleModalLabel').text(modalTitle);
                $('#roleName').val(response.role.name);

                // Clear previous permissions and populate dynamically
                const permissionsTableBody = $('.permissions-section tbody');
                permissionsTableBody.empty();
                response.permissions.forEach(function (permission) {
                    const module = permission.module;
                    const actions = ['Manage', 'Create', 'Edit', 'Delete'];

                    let row = `<tr><td>${module}</td>`;
                    actions.forEach(function (action) {
                        const actionKey = `${action} ${module}`;
                        const isChecked = response.role.permissions.some(p => p.name === actionKey) ? 'checked' : '';
                        row += `
                            <td>
                                <div class="form-check">
                                    <input type="checkbox" name="permissions[]" value="${actionKey}" class="form-check-input" ${isChecked}>
