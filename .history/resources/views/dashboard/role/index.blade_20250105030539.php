...
{{-- Edit Role  --}}
{{ Form::open(['url' => 'roles', 'method' => 'post', 'id' => 'roleForm']) }}
<div class="modal-body">
    <input type="hidden" name="_method" id="formMethod" value="POST"> <!-- For PUT request on edit -->
    <div class="mb-4">
        <label for="roleName" class="form-label">{{ __('Role Name') }}</label>
        {{ Form::text('name', null, ['class' => 'form-control', 'id' => 'roleName', 'placeholder' => __('Enter Role Name')]) }}
    </div>
    ...
    <div class="permissions-section">
        <h5>{{ __('Assign Permissions to Role') }}</h5>
        <table class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>{{ __('Permission') }}</th>
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.role && response.permissions) {
                ...
                $('.permissions-section tbody').empty();
                ...
                response.permissions.forEach(function (permission) {
                    ...
                    $('.permissions-section tbody').append(row);
                });
