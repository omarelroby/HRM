...
<!-- Edit Modals (Outside Container) -->
@foreach($employees as $employee)
<div class="modal fade" id="edit_employee_{{ $employee->id }}" tabindex="-1" aria-labelledby="editEmployeeLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('employee.profile', $employee->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <select class="form-select" id="type" name="role" required>
                                @foreach ($roles as $role)
                                    <option @if($role->name == $employee->user->type) selected @endif value="{{ $role->id }}">{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<!-- Delete Confirmation Modal (Outside Container) -->
