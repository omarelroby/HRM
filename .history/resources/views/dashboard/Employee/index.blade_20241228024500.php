...
<script>
 // Open the Edit Modal and Fetch Employee Data
$(document).on('click', '.edit_employee_modal', function () {
    const id = $(this).data('id'); // Get employee ID from the button

    $.ajax({
        url: `/employee/${id}/edit`, // Laravel resource route for edit
        method: 'GET',
        beforeSend: function () {
            $('#edit_employee').addClass('loading'); // Optional: show loading state
        },
        success: function (response) {
            // Populate modal fields with the fetched data
            $('#edit_employee_id').val(id);
            $('#edit_name').val(response.employee.name);
            $('#edit_name_ar').val(response.employee.name_ar);
...
$('#edit_employee_form').on('submit', function (e) {
    ...
    const id = $('#edit_employee_id').val(); // Get employee ID
    const formData = $(this).serialize(); // Serialize form data

    $.ajax({
        url: `/employee/${id}`, // Laravel resource route for update
        method: 'PUT',
        data: formData,
        success: function (response) {
            alert('Employee updated successfully');
            $('#edit_employee').modal('hide'); // Close the modal
