...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var earlyArrivals = @json() $early_arrivals ?? 0 );
        var earlyArrivals = @json() $late_arrivals ?? 0 );
        var earlyArrivals = @json() $absent_employees ?? 0 );
        var lateArrivals = {{ $late_arrivals ??0 }};
        var absentEmployees = {{ $absent_employees ??0 }};
         // Chart options
        var options = {
            chart: {
                type: 'donut',
