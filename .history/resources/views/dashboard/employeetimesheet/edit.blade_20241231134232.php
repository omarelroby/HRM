                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select id="employee_id" name="employee_id" class="form-select" required>
                                ...
                                @foreach($employees as $id => $name)
                                    <option @if($timeSheet->employee_id == $name->id) selected @endif value="{{ $name->id }}">{{ $name->name ??' '}}</option>
                                @endforeach
                            </select>
                        </div>
                        @endif
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" id="date" value="{{ $timesheet->date }}" name="date" class="form-control datepicker" required>
                        </div>

                        <!-- Hours Input -->
                        <div class="col-md-4 mb-3">
                            <label for="hours" class="form-label">{{ __('Hours') }}</label>
                            <input type="number" id="hours" value="{{ $timesheet->hours }}" name="hours" class="form-control" required step="0.01">
                        </div>

                        <!-- Remarks Input -->
                        <div class="col-md-6 mb-3">
                            <label for="remark" class="form-label">{{ __('Remark') }}</label>
                            <textarea id="remark" name="remark" value="{{ $timesheet->remark }}" class="form-control" rows="2"></textarea>
                        </div>

                        <!-- Arabic Remarks Input -->
                        <div class="col-md-6 mb-3">
                            <label for="remark_ar" class="form-label">{{ __('Remark_ar') }}</label>
                            <textarea id="remark_ar" name="remark_ar" value="{{ $timesheet->remark_ar }}" class="form-control" rows="2"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="button" class="btn btn-secondary" value="{{ $timesheet->remark_ar }}" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    </div>
                </div>
            </form>
        </div>
